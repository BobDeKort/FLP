...
class MyToursViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout, MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        ApiService.sharedInstance.getPurchasedTours { (tours) in
            self.tours = tours
        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: view.frame.width - 26, height: 145)
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let tours = tours {
            ...
            let purchasedTourDetailController = PurchasedTourDetailController(collectionViewLayout: layout)
            purchasedTourDetailController.tour = tours[indexPath.item]
            purchasedTourDetailController.hidesBottomBarWhenPushed = true
            
            navigationController?.pushViewController(purchasedTourDetailController, animated: true)
        }
    }
}
